...
template <char delimiter, char quote_char> struct chunk_cells {
  static const char *chunk(const char *begin, const char *dead_end) {
...
template <class T, class chunk_policy> class fw_iterator {
  const char *begin_;
  const char *end_;
...
public:
  ...
  fw_iterator(const char *begin, const char *dead_end)
      : begin_(begin), end_(chunk_policy::chunk(begin, dead_end)),
        dead_end_(dead_end) {}
  ...
  bool operator!=(const fw_iterator &rhs) const { return begin_ != rhs.begin_; }

  bool operator==(const fw_iterator &rhs) const { return begin_ == rhs.begin_; }

  T operator*() const { return {begin_, end_}; }
...
template <char character> struct quote_char {
  constexpr static char value = character;
};

template <bool flag> struct has_header {
  constexpr static bool value = flag;
};

template <char... Trim_chars> struct trim_chars {
private:
...
public:
  constexpr static std::pair<const char *, const char *> trim(const char *begin,
                                                              const char *end) {
...
#if defined(_WIN32)
    fd_ = CreateFileA(path.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
...
public:
  ...
  private:
    static const char *escape_leading_quote(const char *begin,
                                            const char *end) {
    ...
    static const char *escape_trailing_quote(const char *begin,
                                             const char *end) {
      if (end - begin >= 2 && *begin == quote_char::value &&
          *(end - 1) == quote_char::value)
        return end - 1;
  ...
  using cell_iterator = detail::fw_iterator<
      cell, detail::chunk_cells<delimiter::value, quote_char::value>>;
  ...
  public:
    ...
    cell_iterator begin() const { return {begin_, end_}; }

    cell_iterator end() const { return {end_ + 1, end_ + 1}; }
  };
